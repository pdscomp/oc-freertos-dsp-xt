...
#include "../../include/printer/autoconf.h" // CONFIG_USB_VENDOR_ID
#include "../../include/generic/pgm.h" // PROGMEM
#include "../../include/generic/usb_cdc_ep.h" // USB_CDC_EP_BULK_IN
#include "../../include/printer/byteorder.h" // cpu_to_le16
#include "../../include/printer/command.h" // output
#include "../../include/generic/usbstd.h" // struct usb_device_descriptor
#include "../../include/generic/usbstd_cdc.h" // struct usb_cdc_header_descriptor
#include "../../include/printer/scheder.h" // sched_wake_task
...
void console_sendf(const struct command_encoder *ce, va_list args)    //-sendf-G-G-2022-07-21---
{
...
void usb_bulk_out_task(void)        //usb_read
{
    if (!sched_check_wake(&usb_bulk_out_wake))
        return;
    #if MSGBOX_EN_IRQ
    // msgbox_receive_signal_cb();
    #endif
    ...
    if (rpos + USB_CDC_EP_BULK_OUT_SIZE <= sizeof(receive_buf)) {
        int_fast8_t ret = usb_read_bulk_out(&receive_buf[rpos], USB_CDC_EP_BULK_OUT_SIZE);   //-----ggg---6.9
        if (ret > 0) {
            rpos += ret;
            usb_notify_bulk_out();
        }
...
{
//     if (req->bRequestType != USB_DIR_IN)  ///-----ggg----6.9
//         goto fail;
//     void *desc = NULL;
...
static void usb_req_set_configuration(struct usb_ctrlrequest *req)
{
