...
#include "../../include/board/gpio.h" // struct gpio_adc
...
analog_in_event(struct timer *timer)
{
    ...
    uint32_t sample_delay = 0;//gpio_adc_sample(&(a->pin));  //--G-G-222-07-29---
    if (sample_delay) {
        a->timer.waketime += sample_delay;
        return SF_RESCHEDULE;
    }
    uint16_t value = gpio_adc_read(&(a->pin));  //2000--G-G-222-07-29--- value >=1344
    ...
    // hal_gpio_set_data(196, 0);          //--G-G-222-07-29---
...
{
    struct analog_in *a = oid_alloc(args[0], command_config_analog_in, sizeof(struct analog_in));
    if(gpio_adc_setup(&(a->pin),args[1]-(32+13)) < 0)   //--PB13-PB15----G-G-2022-07-30------
    {
        try_shutdown("ADC out of range");
    }
...
command_query_analog_in(uint32_t *args)
{
    ...
    if (! a->sample_count)
        return;
   if(sched_add_timer(&a->timer))
    {
        GAM_DEBUG_errer( "--timer_reset--2----" );
    }
