...
#include "../../include/hal_call/spicmds.h" // spidev_transfer
...
adxl_query(struct adxl345 *ax, uint8_t oid)
{
    ...
    if (fifo_status > 1 && fifo_status <= 32) {
        ...
        if(sched_add_timer(&ax->timer))
        {
            GAM_DEBUG_errer( "--timer_reset--8----" );
        }
        irq_enable_all();
    }
}
...
command_query_adxl345(uint32_t *args)
{
    ...
    if(sched_add_timer(&ax->timer))
    {
        GAM_DEBUG_errer( "--timer_reset--10----" );
    }
}
