...
#include "../../include/printer/autoconf.h" // CONFIG_*
#include "../../include/printer/basecmd.h" // stats_update
...
#include "../../include/printer/scheder.h" // sched_check_periodic
#include "../../include/printer/stepper.h" // stepper_event
...
int sched_add_timer(struct timer *add)
{
    uint32_t waketime = add->waketime;
    irqstatus_t flag = irq_save();
    if (unlikely(timer_is_before(waketime, timer_list->waketime))) {    // This timer is before all other scheduled timers
        ...
        timer_set_diff(waketime-cur);       // 
        // timer_kick();            //这里似乎有个BUG 会立即调用
    }
    else {
...
run_tasks(void)
{
 ...
 #if MSGBOX_EN_SYNC
                    irq_wait();  //-----ggg----
                    if (msgbox_new_msg[1] >= 5000)
                    {
                        GAM_DEBUG_printf("run_tasks wait msg  %d %d\n",msgbox_new_msg[0],msgbox_new_msg[1] );
                    }
                    else
#elif SHARE_SPACE_SYNC
                    static uint8_t share_space_set_read_idle_need = 0;
                    irq_wait();  //-----ggg----
                   if (msgbox_new_msg[SHARESPACE_WRITE] == sharespace_arm_addr[SHARESPACE_WRITE] )
                    {
                        if(share_space_set_read_idle_need != 2)
                            share_space_set_read_idle_need = share_space_set_read_idle_state();
                    }
                    else
#else
                     irq_wait();  //-----ggg----
                    // if ((msgbox_new_msg[1] < 4096) && (msgbox_new_msg[1] > 11) && (msgbox_new_msg[1] != sharespace_dsp_addr[0] )  )
#endif
                    {
#if SHARE_SPACE_SYNC
                         share_space_set_read_idle_need = 0;
 #endif
...
run_shutdown(int reason)
{
    ...
    // share_space_init();      //---G-G-2022-08-03----
...
{
    irq_disable_all();
    printf("111-%d\n",reason);   //ctr_lookup_static_string
...
void sched_main(void)
{
    ...
    irq_enable_all();
    run_tasks();
    receive_msg_queue_free();
}

FINSH_FUNCTION_EXPORT_CMD(sched_main, sched_main, sched main);