...
#include "../../include/board/misc.h" // timer_is_before
...
#include "../../include/printer/trsync.h" // trsync_add_signal
...
void command_config_stepper(uint32_t *args)
{
    ...
    printf("config_stepper oid=%d step_pin=%d dir_pin=%d invert_step=%d step_pulse_ticks=%u\n", args[0], args[1], args[2], args[3], args[4]);
}
...
void command_queue_step(uint32_t *args)
{
    ...
    m->add = args[3];
    m->flags = 0;
    // printf("queue_step oid %d interval %u count %u add %d\n", args[0], m->interval, m->count, m->add);
    irq_disable_all();
