...
#include "../../include/board/gpio.h" // struct gpio
#include "../../include/board/irq.h" // irq_disable
#include "../../include/board/misc.h" // timer_read_time
#include "../../include/printer/command.h" // DECL_COMMAND
#include "../../include/printer/scheder.h" // struct timer
#include "../../include/printer/trsync.h" // trsync_do_trigger
...
// Report handler
static uint_fast8_t trsync_report_event(struct timer *t)
{
    // hal_gpio_set_data(196, 0);          //--G-G-222-07-29---
...
void command_trsync_start(uint32_t *args)
{
    ...
    if (ts->report_ticks)
    {
        if(sched_add_timer(&ts->report_time))
        {
            GAM_DEBUG_errer( "--timer_reset--14----" );
        }
    }
...
void command_trsync_set_timeout(uint32_t *args)
{
    ...
    if (flags & TSF_CAN_TRIGGER) {
        ...
        if(sched_add_timer(&ts->expire_time))
        {
            GAM_DEBUG_errer( "--timer_reset--13----" );
        }
