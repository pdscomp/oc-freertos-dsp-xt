...
#include "../../include/printer/basecmd.h" // oid_lookup
#include "../../include/board/irq.h" // irq_save
#include "../../include/board/misc.h" // alloc_maxsize
#include "../../include/generic/pgm.h" // READP
#include "../../include/printer/command.h" // DECL_COMMAND
#include "../../include/printer/scheder.h" // sched_clear_shutdown
...
static void *alloc_chunks(size_t size, size_t count, uint16_t *avail)
{
    ...
    while (can_alloc < count && p + size <= end)
        can_alloc++, p += size;
    if (can_alloc<count)    //     if (!can_alloc)
    {
        printf("-%d-%d-%d---\n",can_alloc, size,count );
...
static void move_finalize(void)
{
    ...
    move_list = alloc_chunks(move_item_size, 2048, &move_count);        //16KB
    move_reset();
}
...
command_get_uptime(uint32_t *args)
{
    ...
    sendf("uptime high=%u clock=%u", high, cur);               //-sendf-G-G-2022-07-21---
    // timer_printf("cur = %u\n", cur);
    // timer_printf("high = %u\n", high);
}
