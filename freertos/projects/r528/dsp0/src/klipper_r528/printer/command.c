...
#include "../../include/board/io.h" // readb
#include "../../include/board/irq.h" // irq_poll
#include "../../include/board/misc.h" // crc16_ccitt
#include "../../include/generic/pgm.h" // READP
#include "../../include/printer/command.h" // output_P
#include "../../include/printer/scheder.h" // sched_is_shutdown
#include <stdio.h>
...
static uint32_t parse_int(uint8_t **pp)
{
    ...
    uint32_t v = c & 0x7f;
    if ((c & 0x60) == 0x60)
        v |= -0x20;     //第一字节大于 0x60 是符号位
    while (c & 0x80) {    //  小于 0x80 是结束标识
        c = *p++;
        v = (v<<7) | (c & 0x7f);
    }
    *pp = p;
...
// Parse an incoming command into 'args'  //-----gg-5-----------
...
// Encode a message
uint_fast8_t command_encodef(uint8_t *buf, const struct command_encoder *ce, va_list args)       //-sendf-G-G-2022-07-21---
{
...
void command_add_frame(uint8_t *buf, uint_fast8_t msglen)    //-sendf-G-G-2022-07-21---
{
...
uint_fast8_t command_encode_and_frame(uint8_t *buf, const struct command_encoder *ce  , va_list args)    //-sendf-G-G-2022-07-21---
{
...
void command_sendf(const struct command_encoder *ce, ...)     //-sendf-G-G-2022-07-21---
{
    if (readb(&in_sendf))
    {
        share_space_printf("This sendf call was made from an irq handler\n");
...
command_lookup_parser(uint_fast8_t cmdid) //-----gg-4-----------
{
...
int_fast8_t command_find_block(uint8_t *buf, uint_fast8_t buf_len, uint_fast8_t *pop_count)
{
    ...
    if (buf_len < MESSAGE_MIN)
    {
        if (buf_len )
        {
            GAM_DEBUG_errer("-need_more_data:%d--\n",buf_len);
            GAM_DEBUG_printf_HEX1(0,buf,buf_len);
        }
    ...
    if (msglen < MESSAGE_MIN || msglen > MESSAGE_MAX)
     {
        GAM_DEBUG_errer("-error msglen:%d--\n",msglen);
        goto error;
     }
    uint_fast8_t msgseq = buf[MESSAGE_POS_SEQ];
    if ((msgseq & ~MESSAGE_SEQ_MASK) != MESSAGE_DEST)
    {
        GAM_DEBUG_errer("-error msgseq:%d--\n",msgseq);
        goto error;
    }
    if (buf_len < msglen)
    {
        GAM_DEBUG_errer("-need_more_data:%d-%d-\n",buf_len,msglen);
        goto need_more_data;
    }
    if (buf[msglen-MESSAGE_TRAILER_SYNC] != MESSAGE_SYNC)
   {
        GAM_DEBUG_errer("-error MESSAGE_SYNC:%x-\n",buf[msglen-MESSAGE_TRAILER_SYNC]);
        goto error;
   }
    ...
    if (crc != msgcrc)
   {
        GAM_DEBUG_errer("-error crc:%d-%d-\n",crc,msgcrc);
        goto error;
   }
    sync_state &= ~CF_NEED_VALID;
    *pop_count = msglen;
    if (msgseq != next_sequence) {
        GAM_DEBUG_errer("-nak:%x-%x-\n",msgseq,next_sequence);
        GAM_DEBUG_printf_HEX1(0,buf,buf_len);
        goto nak;
    }
...
//-len-cmdordinal-cmdid-args-CRC16-7e----------------------
void command_dispatch(uint8_t *buf, uint_fast8_t msglen) //-----gg-3-----------
{
    uint8_t *p = &buf[MESSAGE_HEADER_SIZE];
    ...
    while (p < msgend) {
        ...
        // if (sched_is_shutdown() && !(READP(cp->flags) & HF_IN_SHUTDOWN)) {  //--gg-7.4
...
int_fast8_t command_find_and_dispatch(uint8_t *buf, uint_fast8_t buf_len  , uint_fast8_t *pop_count)
{           //-----gg-2-----------
