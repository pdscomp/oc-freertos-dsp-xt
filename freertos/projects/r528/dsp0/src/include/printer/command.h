...
#define sendf(FMT, args...)                     command_sendf(_DECL_ENCODER(FMT) , ##args )            //-sendf-G-G-2022-07-21---

// Shut down the machine (also declares a static string to transmit)
#define shutdown(msg)             sched_shutdown(_DECL_STATIC_STR(msg))
#define try_shutdown(msg)      sched_try_shutdown(_DECL_STATIC_STR(msg))
...
#define _DECL_ENCODER(FMT) ({                     DECL_CTR("_DECL_ENCODER " FMT);            ctr_lookup_encoder(FMT); })      //-sendf-G-G-2022-07-21---

#define _DECL_OUTPUT(FMT) ({                    \
    DECL_CTR("_DECL_OUTPUT " FMT);              \
    ctr_lookup_output(FMT); })

// #define _DECL_STATIC_STR(MSG) ({    DECL_CTR("_DECL_STATIC_STR " MSG);              printf("Shutdown: %s,%d\n", MSG,ctr_lookup_static_string(MSG)); ctr_lookup_static_string(MSG); })
#define _DECL_STATIC_STR(MSG) ({    DECL_CTR("_DECL_STATIC_STR " MSG);       ctr_lookup_static_string(MSG); })
